...
- JPEG passthrough copies, PNG output, and extracted-image placement results now
  grow fallibly. PNG allocation refusal is no longer mistaken for an ordinary
  codec failure that could silently omit an image, and image format tags no
...
- `Document.compress_images()` now rejects more than 65,536 interpreted
  ...
  `HashMap` insertion or iterator collection. Candidate and mask-reference sets
  are likewise allocated fallibly before any compression edit.
