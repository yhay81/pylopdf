...
because pylopdf currently accepts paths or complete byte strings. Cloudflare's
128 MiB isolate budget also includes Python, JavaScript, WebAssembly linear
memory, and request buffers, so production applications should reduce the file
budget when their surrounding code needs more headroom.
