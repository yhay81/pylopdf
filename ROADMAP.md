...
- 上流貢献（2026-07-23 に着手）: lopdf#535 の修正 PR を提出済み
  ...
  content パーサが以降を全部落とすバグの 1 行修正 + 回帰テスト）。hayro へは
  性能 issue [#1315](https://github.com/LaurenzV/hayro/issues/1315)（ステンシル
  マスク画像が約 5 倍遅）と機能提案 [#1316](https://github.com/LaurenzV/hayro/issues/1316)
  （RenderSettings への num_threads 公開）を提出し、両方の修正 PR を作業中
  （引き継ぎ時点の状態: num_threads は実装 + 検証完了（clippy/fmt/テスト/6 PDF・
  147 ページのピクセルビット一致）で cargo hack 確認 → push 直前、ブランチ名
  yhay81:expose-render-num-threads。ステンシルマスクは検証途中、ブランチ名
  yhay81:optimize-stencil-mask-images。提出済みかは
  `gh pr list --repo LaurenzV/hayro --author yhay81` で確認し、未提出なら
  issue 本文の実測・方針から再開できる）。
