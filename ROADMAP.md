...
- [ ] README（両言語）にエコシステム連携節: 組版・新規文書の PDF/A = typst-py、
      電子署名 = pyHanko、PDF/A 検証 = veraPDF 外部委譲の案内
- [ ] 連携レシピの統合テスト（optional dependency-group で typst / pyHanko を入れ、
      `typst.compile → pylopdf.open(stream=)` と「pylopdf 出力 → pyHanko 増分署名で
      元バイトが無加工で保たれる」ことを検証）
...
- 日本語テキストの描き込み（透かし・ヘッダ/フッタの CJK 対応）: 標準 14 フォントでは
  不可能なので、krilla のフォント埋め込み（CID + サブセット化、typst で実績）を検討。
  経路は「krilla で 1 ページ生成 → hayro-write で XObject 化 → lopdf へ挿入」。
  feature 最小化なら wheel 増 1MB 未満の見込み。制約: krilla の `pdf` feature は
  hayro-syntax の版に固定されるため Cargo.lock で単一バージョン解決が必須。
  hayro 0.8 追従とは順序を分けて別コミットにする
...
- OCR 結果の不可視テキスト層書き込みプリミティブ（語 + bbox を受けて searchable PDF 化。
  どの OCR とも組める中立 API。to_markdown のスキャン PDF 対応と v0.10 [ocr] の土台）
- XMP の PDF/A 宣言（pdfaid:part / conformance）の読み取り。「検証」ではないことを
  API 名で明示（本検証は veraPDF へ委譲）
...
- 配布 = `pylopdf-ocr-models` 別 wheel（fonts-cjk と同じパターン。モデル世代を本体と
  独立に更新できる）
- 前提条件: tests/assets/real_world 級のコーパスで日本語精度を実測してから公開を判断。
  参考実装 ocrs-cjk（MIT/Apache。rten + PP-OCRv5 + 不可視テキスト層まで実装済みだが
  スター 2 の超初期）は依存にせずコード参考に留める
